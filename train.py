...
def main():
    ...
    optimizer_D_A = torch.optim.Adam(
        D_A.parameters(), lr=args.lr, betas=(args.beta1, 0.999)
    )
    optimizer_D_B = torch.optim.Adam(
        D_B.parameters(), lr=args.lr, betas=(args.beta1, 0.999)
    )
    ...
    for epoch in range(total_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            loss_G_B2A = adv_loss(
                pred_fake_A, torch.tensor(1.0).expand_as(pred_fake_A).to(device)
            )

            pred_fake_B = D_B(fake_B)
            loss_G_A2B = adv_loss(
                pred_fake_B, torch.tensor(1.0).expand_as(pred_fake_B).to(device)
            )
            ...
            if args.lambda_identity > 0:
                ...
                loss_G = (
                    loss_G_A2B
                    + loss_G_B2A
                    + loss_cycle_A * args.lambda_cycle
                    + loss_cycle_B * args.lambda_cycle
                    + loss_identity_A * args.lambda_cycle * args.lambda_identity
                    + loss_identity_B * args.lambda_cycle * args.lambda_identity
                )
            ...
            loss_D_A_real = adv_loss(
                pred_real_A, torch.tensor(1.0).expand_as(pred_real_A).to(device)
            )
            loss_D_A_fake = adv_loss(
                pred_fake_A, torch.tensor(0.0).expand_as(pred_fake_A).to(device)
            )
            ...
            loss_D_B_real = adv_loss(
                pred_real_B, torch.tensor(1.0).expand_as(pred_real_B).to(device)
            )
            loss_D_B_fake = adv_loss(
                pred_fake_B, torch.tensor(0.0).expand_as(pred_fake_B).to(device)
            )
        ...
        if (epoch + 1) % args.checkpoint_frequecy == 0:
            ...
            torch.save(
                D_A.state_dict(), "models/" + args.model + "/D_A/" + str(epoch) + ".pth"
            )
            torch.save(
                D_B.state_dict(), "models/" + args.model + "/D_B/" + str(epoch) + ".pth"
            )
